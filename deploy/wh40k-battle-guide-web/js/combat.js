import { leadershipRoll, requiredWoundRoll, rollD6 } from './dice.js';

export function resolveWeaponAttacks(weapon, attackerModels, targetUnit, hitModifier = 0, cover = false) {
  const diceLog = [];
  let totalAttacks = 0;
  ...
  for (let i = 0; i < wounds; i++) {
    ...
    if (inflicts) {
      ...
      diceLog.push(
        `${target.name} takes ${weapon.damage} damage (${target.woundsRemaining} W remaining)`,
      );
...
export function resolveBattleShock(unit) {
...
export function getRangedWeapons(unit) {
...
export function getMeleeWeapons(unit) {
  const weapons = new Map();
  for (const model of unit.models) {
    for (const w of model.weapons) {
      if (w.range === 'melee') weapons.set(w.id, w);
