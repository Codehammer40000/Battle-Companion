"""Convert Rapid Ingress PF-TH-C into Battle Companion layout JS (from RapidIngress-pfthc.png slug)."""
...
# Any saved Rapid Ingress _files copy has the full 11e layout catalog
SRC = (
    ROOT
    / "Purge the Foe vs Take and Hold - Layout A _ Warhammer 40k Terrain Layout _ Rapid Ingress_files"
    / "terrain-data-11e.js.download"
)
OUT = ROOT / "app" / "js" / "battleSim" / "layoutData" / "purgeVsTakeAndHoldC.js"
LAYOUT_ID = "PF-TH-C"
...
def main() -> None:
    ...
    src = next(l for l in layouts if l["id"] == LAYOUT_ID)
    ...
    layout = {
        "id": "purge-vs-take-and-hold-c",
        "name": "Purge vs Take and Hold C",
        "page": 4,
        "source": "Rapid Ingress PF-TH-C (from RapidIngress-pfthc.png slug)",
    ...
    OUT.write_text(
        "/** Auto-generated from Rapid Ingress PF-TH-C — do not hand-edit polygons. */\n"
        f"export const PURGE_VS_TAKE_AND_HOLD_C_LAYOUT = {body};\n",
        encoding="utf-8",
    )
    print(
        "wrote",
