...
/**
 * Models still alive given wounds taken against the unit.
 * Mixed profiles remove weakest models first when a model's full W is spent.
 */
export function getRemainingModelsFromUnit(unit, woundsTaken) {
  ...
  if (Array.isArray(comp) && comp.length) {
    ...
    for (const c of comp) {
      ...
      for (let i = 0; i < n; i++) pools.push(w);
    }
    if (!pools.length) return 0;
    pools.sort((a, b) => a - b);
    let remaining = dmg;
    let alive = pools.length;
...
function parseUnit(sel) {
  ...
  return {
    ...
    woundsPerModel: majorityWounds || parseWoundValue(stats.W),
    modelComposition,
    totalWounds: compositionWounds || majorityWounds * modelCount,
    keywords,
    stats,
