...
export function getUnitWoundCapacity(unit) {
  return getUnitWoundCapacityFromUnit(unit);
}

export function getRemainingModels(unit, woundsTaken) {
  return getRemainingModelsFromUnit(unit, woundsTaken);
}
...
function syncBattleMapModels(state) {
  ...
  for (const [key, entry] of Object.entries(unitsOnMap)) {
    ...
    const group = getUnitGroupInfo(army, attachmentsByPlayer[entry.player], entry.unitId);
    const unitIds = [
      ...new Set([
        entry.unitId,
        ...(group.unitIds || []),
        ...models.map((m) => m.unitId).filter(Boolean),
      ]),
    ];
    ...
    for (const uid of unitIds) {
      ...
      // Leader models left on this blob after detach → promote to own map entry
      if (uid !== entry.unitId) {
        const attachedTo = attachmentsByPlayer[entry.player]?.[uid];
        if (attachedTo !== entry.unitId) {
          const leaderModels = own();
          if (leaderModels.length) {
            models = others();
            entryChanged = true;
            const leaderKey = mapUnitKey(entry.player, uid);
            const existing = unitsOnMap[leaderKey];
            unitsOnMap[leaderKey] = {
              player: entry.player,
              unitId: uid,
      ...
      if (mine.length < remaining) {
        ...
        for (let i = mine.length; i < remaining; i++) {
          // Attached multi-model leaders: index 0 is leader/support-sized
          let role = 'standard';
          if (uid !== entry.unitId) {
            role =
              i === 0
...
export function guideReducer(state, action) {
  switch (action.type) {
    ...
    case 'SET_LEADER_ATTACHMENT': {
      ...
      // If the bodyguard is already on the map, add/remove attached leader models
    ...
    case 'MAP_DEPLOY_UNIT': {
      ...
      const attachedLeaders = group.unitIds
        ...
        .map((u) => ({
          ...u,
          _remainingModels: getRemainingModels(u, getUnitWoundsTaken(state, player, u.id)),
        }));
